...
public class HorizontalBarChartRenderer extends BarChartRenderer {
    ...
    public HorizontalBarChartRenderer(BarDataProvider chart, @Nullable ChartAnimator animator, ViewPortHandler viewPortHandler) {
    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        for (int j = 0, pos = 0; j < buffer.size(); j += 4, pos++) {
            ...
            if (isCustomFill) {
                dataSet.getFill(pos).fillRect(c, mRenderPaint, buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2], buffer.buffer[j + 3], isInverted ? Fill.Direction.LEFT : Fill.Direction.RIGHT);
            } else {
    ...
    protected boolean isDrawingValuesAllowed(ChartInterface chart) {
        return chart.getData().getEntryCount() < chart.getMaxVisibleCount() * mViewPortHandler.getScaleY();
    }
}
