...
public class LineChartRenderer extends LineRadarRenderer {
    ...
    public LineChartRenderer(LineDataProvider chart, @Nullable ChartAnimator animator, ViewPortHandler viewPortHandler) {
    ...
    protected void drawLinear(Canvas c, ILineDataSet dataSet) {
        ...
        if (dataSet.getColors().size() > 1) {
            ...
            for (int j = mXBounds.min; j < max; j++) {
                ...
                // bounds
                if (!mViewPortHandler.isInBoundsLeft(lastCoordinateX) || !mViewPortHandler.isInBoundsTop(Math.max(firstCoordinateY, lastCoordinateY)) || !mViewPortHandler.isInBoundsBottom(Math.min(firstCoordinateY, lastCoordinateY)))
                    continue;
